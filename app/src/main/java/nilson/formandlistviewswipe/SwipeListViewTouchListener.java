...
/**
 * Copiado de:
 * http://stackoverflow.com/questions/35886730/how-to-start-activities-by-swiping-different-listview-items-in-android
 * on 23/03/2016.
 */
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    public interface OnSwipeCallback {
         ...
         * @param pos                    AA posicao que vai fazer alguma coisa
         *
         */
        void onSwipeLeft(ListView listView, int pos);

        void onSwipeRight(ListView listView, int pos);
    }

    public interface OnSwipeCallback2 {
        /**
         * Called when the user has swiped the list item to the left.
         *
         * @param listView               The originating {@link ListView}.
     ...
     * impede que o swip seja executado sem querer enquanto se esta navegando na listview
     *
     * @param enabled Whether or not to watch for gestures.
     */
    public void setEnabled(boolean enabled) {
        mPaused = !enabled;
        //Log.e("VV","teste4 - setEnabled="+enabled);
    }
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (motionEvent.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
            {
                ...
                //Log.e("VV","teste6 - childCount="+childCount);
                ...
                if (mDownView != null) {
                    ...
                    //Log.e("VV", "teste7 - posicao="+mDownPosition);
            ...
            case MotionEvent.ACTION_UP:
            {
                if (mVelocityTracker == null) {
                    //esta variavel eh setada ao se pressionar uma posicao, quando ela eh nula
                    //eh pq a posicao eh nula (nao tem nenhum elemento nessa posicao)
                    //quando se esta soltando uma posicao nula - nenhuma acao eh executada
                    //Log.e("VV","teste8");
                    break;
                }

                float deltaX = motionEvent.getRawX() - mDownX;
                //Nilson -> Isso aqui impede de swipar pra a esqueda
                //Log.e("VV",deltaX+"");
                if(deltaX<0) {
                    // cancel
                    mDownView.animate()
                ...
                //quanto menor o numero do dividendo mais movimento tem que fazer
                //Log.e("VV",""+Math.abs(deltaX)+" > "+(mViewWidth / 1.9)+" ?");
                //Log.e("VV","Min="+mMinFlingVelocity+" | Max="+mMaxFlingVelocity+" | Atual="+velocityX);
                if (Math.abs(deltaX) > mViewWidth / 1.9 ) {
                    swipe = true;
                    swipeRight = deltaX > 0;
                } else if (mMinFlingVelocity <= velocityX && velocityX <= mMaxFlingVelocity && velocityY < velocityX) {
                    if (Math.abs(deltaX) > mViewWidth / 4 ) {
                        swipe = true;
                ...
                if (swipe) {
                    ...
                    mDownView.animate()
                            ...
                            .alpha(0)//fica invisivel ao largar o dedo
                            .setDuration(mAnimationTime)
    ...
    private void performSwipeAction(final View swipeView, final int swipePosition, boolean toTheRight, boolean dismiss) {
        ...
        //Log.d("VV SAMPLE", "SAMPLE IN SWIPE -- " + swipePosition);

        if (swipeRight) {
            mCallback.onSwipeRight(mListView, swipePosition);
        } else {
            mCallback.onSwipeLeft(mListView, swipePosition);
        }
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mDismissAnimationRefCount == 0) {
                    ...
                    /*
                    if (swipeRight) {
                        mCallback.onSwipeRight2(mListView, swipePosition);
                    } else {
                        mCallback.onSwipeLeft2(mListView, swipePosition);
                    }
                    */
